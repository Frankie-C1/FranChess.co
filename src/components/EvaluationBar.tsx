...
export function EvaluationBar({ cp, mate }: { cp: number | null; mate: number | null }) {
  ...
  return (
    <div className="relative flex h-8 w-full overflow-hidden rounded-md border border-stone-200 bg-stone-950 text-xs font-semibold shadow-sm dark:border-stone-800 sm:h-full sm:w-9 sm:flex-col">
      ...
      <span className="absolute left-1/2 top-1/2 block -translate-x-1/2 -translate-y-1/2 rounded bg-white/80 px-1 text-stone-900 sm:hidden">
        {formatEval(cp, mate)}
      </span>
    </div>
  );
